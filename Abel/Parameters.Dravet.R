...
# Parameters ------------------------
{
  ...
  p$'cellWhiteList'                 = '~/Dropbox (VBC)/Abel.IMBA/MetadataD/INM.meta/cell.lists/' # List of cells taken for the analysis in a 2 column table: (cellID - sampleID)
  p$'subclustering'                 = F                        # Is this run sub-clustering run?
  p$"file.ext"                      = c("pdf", "png", "jpg")[2]     # Save plots in what default format?

  # Filtering parameters -----
  # LP for low-pass (below), HP for high-pass (above)
  p$'min.cells'             = 10
  p$'min.features'          = 200
  p$'thr.lp.nFeature_RNA'   = 10000
  p$'thr.hp.nFeature_RNA'   = 1000
  p$'thr.hp.mito'           = 0.005                   # Minimal fractions of mitochondrial reads in total transcriptome.
  p$'thr.lp.mito'           = 0.15
  p$'thr.hp.ribo'           = 0.0
  p$'thr.lp.ribo'           = 0.2
  ...
  # p$'variables.2.regress'   = 'nFeature_RNA' # NULL # c( 'percent.mito', 'percent.ribo', 'nFeature_RNA')
  # p$'variables.2.regress'   = c(  'percent.ribo', 'nFeature_RNA') # 'percent.mito', ''
  ...
  p$'def_res'               = .3       # Which clustering resolutions to calculate

  p$'res.analyzed.DE'       = unique(c(.3, .5)) # Which clustering resolutions to analyze in the differential gene expression script
  ...
  p$'STRING.nr.genes'             = 100 # How many genes should be passed on to STRING
  p$'STRING.max.genes'            = 100 # max genes in a GO term to be meaningful, larger are not used
  p$"getSTRINGlinks"              = F
  assay.def = 'RNA'
